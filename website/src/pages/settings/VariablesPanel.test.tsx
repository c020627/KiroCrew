...
describe('VariablesPanel', () => {
  ...
  describe('global scope', () => {
    it('adds a pair, keeping the pairs already stored', async () => {
      ...
      await waitFor(() => expect(saveVariables).toHaveBeenCalledWith({
        scope: 'global',
        values: { BASE_URL: 'https://example.test', REGION: 'eu-west-1' },
      }))
    })
    ...
    it('edits a value on blur', async () => {
      ...
      await waitFor(() => expect(saveVariables).toHaveBeenCalledWith({
        scope: 'global',
        values: { BASE_URL: 'https://new.test' },
      }))
    })
    ...
    it('deletes a pair by writing the scope without it', async () => {
      ...
      await waitFor(() => expect(saveVariables).toHaveBeenCalledWith({
        scope: 'global',
        values: { BASE_URL: 'https://example.test' },
      }))
    })
    ...
    it('surfaces a per-key rejection the backend returns with a 200', async () => {
      variables.mockResolvedValue(view())
      saveVariables.mockResolvedValue({ error: 'REGION: name is reserved', key: 'REGION' })
      mount()
      await ready()
      ...
      expect(await screen.findByText('REGION: name is reserved')).toBeInTheDocument()
    })
  })
  ...
  describe('workspace scope', () => {
    ...
    it('adds a pair at the named workspace', async () => {
      ...
      await waitFor(() => expect(saveVariables).toHaveBeenCalledWith({
        scope: 'workspace',
        workspace: 'research',
        values: { REGION: 'eu-west-1', STAGE: 'beta' },
      }))
    })
  ...
  describe('client validation, before any round trip', () => {
    ...
    it('accepts a tab, the one control character the backend allows', async () => {
      ...
      await waitFor(() => expect(saveVariables).toHaveBeenCalledWith({
        scope: 'global',
        values: { BASE_URL: 'https://example.test', COLS: 'a\tb' },
      }))
    })
