...
describe('SessionPulseSurveyCard (chat-page-only surface gate)', () => {
  beforeEach(() => {
    localStorage.clear()
    server.use(
      http.get('/api/feedback/eligible', () => HttpResponse.json({ eligible: true })),
  ...
  function renderCardWithSession(sessionId: string) {
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const utils = render(
      <QueryClientProvider client={qc}>
        <SessionPulseSurveyCard
  ...
  it.each([
    ['slack_1786750215.779379', 'a Slack-linked session'],
    ['discord_kirocrew_direct_513187599515385866', 'a Discord-linked session'],
    ['cron-03fbeacd', 'a cron-triggered session'],
    ['exec-ticket-897-g20260814-2201', 'an exec-ticket automation session'],
    ['spec-builder-task-specification-sod-based-intraday-position-a8fe940c', 'a spec-builder task session'],
    ['workflow-abc123', 'a workflow run session'],
  ])('never shows for %s (%s), even past turn 3 with eligible:true', async (sessionId) => {
    const { rerenderWithTurnCount } = renderCardWithSession(sessionId)
    rerenderWithTurnCount(3)
  ...
  it('still shows for an ordinary chat-<n>-<ts> session (the dashboard Chat page)', async () => {
    const { rerenderWithTurnCount } = renderCardWithSession('chat-7-1786950000')
    await showCard(rerenderWithTurnCount)
    expect(screen.getByText(RATING_QUESTION)).toBeInTheDocument()
  })
})
