...
describe('MdNotebookPage — settings, guarded mutations and editor keys', () => {
  ...
  it('persists the autosave and auto-sync switches toggled in Settings', async () => {
    ...
    // the server rather than this browser's storage.
    await userEvent.click(screen.getByRole('switch', { name: 'Auto sync' }))
    await waitFor(() => expect(api.saveSettings).toHaveBeenCalledWith({ autoSync: true }))
  ...
  it('clamps the auto-sync interval into its allowed range before saving it', async () => {
    ...
    // is an interval the backend would otherwise start syncing on.
    await waitFor(() => expect(api.saveSettings).toHaveBeenCalledWith({ autoSyncMins: 10 }), {
      timeout: 3000,
    })
