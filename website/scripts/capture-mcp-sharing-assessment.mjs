...
await stubDashboardApi(page, {
  extra: (path, route) => {
    ...
    if (path === '/api/mcp-gateway/servers/stub') {
      ...
      // button alone cannot show what it reports having done.
      return json(route, { ok: true, names: [], stub: true, applied: true }), true
    }
...
// 7. The bulk action, in the state a reader meets it: the button names what it
...
await page.reload({ waitUntil: 'domcontentloaded' })
await page.waitForTimeout(2400)
await shot('07-stub-what-evidence-allows')

// 8. What one press reports. Sharing is ON, so eligibility takes the verdict that
...
await shot('08-stub-result-what-it-skipped')

// 9. A pass the button itself started, in flight. Nothing is stubbed until the
...
await page.getByRole('button', { name: /evidence allows/i }).click()
await page.waitForTimeout(2600)
await shot('09-stub-waiting-on-measurement')

// 10. The new tier on the evidence view. Its wording is the whole point: MEASURED
