...
await page.route('**/api/**', async route => {
  ...
  // The registry path. For an app no registry lists this is exactly the
  // failure the bug produced, so the stub answers with it. Both the streaming
  // and non-streaming routes are matched: matching only the latter would make
  // the "never reached" assertion below vacuous, since the client uses the
  // streaming one.
  if (path === '/api/apps/registry/install-stream' || path === '/api/apps/registry/install') {
    installStreamCalls += 1
    return json(route, { ok: false, error: `app '${REGISTRY_APP.name}' not found in registry` }, 400)
  }
