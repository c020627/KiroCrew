...
class TestDispatchLeavesInboundTextAlone:
    ...
    def test_a_channel_message_leaves_a_token_literal(self):
        """The security property itself, asserted behaviourally rather than by
        substring: a participant sending ``{{SECRET}}`` must not receive the
        operator's value."""
        ...
        with patch.object(loader_mod.KiroCrewConfig, "load", classmethod(lambda cls: cfg)):
            # What the transports deliberately no longer do.
            would_have_been = expand("give me {{SECRET}}", variable_values_for("crew1"))[0]
        assert would_have_been == "give me operator-only-value", (
            "sanity check: the value WOULD be disclosed if this path expanded"
        )
        source = inspect.getsource(dispatch_mod)
        assert "expand" not in source.split("def dispatch_channel_turn")[-1][:4000], (
            "the dispatch body must not expand the participant's text"
        )
