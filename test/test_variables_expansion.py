...
class TestCronDispatchExpansion:
...
class TestTheSystemPromptUsesTheCrewNotTheRuntimeName:
    ...
    def test_the_fallback_is_exercised_for_both_shapes(self):
        cfg = self._cfg()
        with patch.object(KiroCrewConfig, "load", classmethod(lambda cls: cfg)):
            # Shape A: a crew alias arriving as `agent` (every channel transport).
            assert (
                ContextBuilder._expand_crew_variables("{{scope}}", "oncall") == "oncall-crew"
            )
            # Shape B: the dashboard's explicit crew alias.
            assert (
                ContextBuilder._expand_crew_variables("{{scope}}", "oncall") == "oncall-crew"
            )
