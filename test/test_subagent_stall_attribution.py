...
async def test_working_cannot_suppress_the_badge_forever():
    ...
    with patch("kiro_crew.subagent.LivenessOracle", return_value=oracle), patch(
        "kiro_crew.subagent.record_slow_command"
    ):
...
@pytest.mark.asyncio
async def test_dead_still_skips_two_sweep_for_a_lone_agent_in_a_shared_runtime():
    """`session_sharing` being ON is not itself the hazard — an unshared sibling
    set is. A lone subagent has no sibling to be confused with, so it keeps the
    fast path rather than paying a sweep for a risk it does not carry."""
    mgr = _make_manager(stall_idle_secs=10)
    now = 1_000.0
    info = _info(turns=1, _pid=4242, last_activity=now - 25, _session_sharing=True)
    _register(mgr, info)
    assert mgr._live_shared_count(4242) == 1
    assert await _flag_with_dead(mgr, info, now) is True, "lone agent lost its fast path"
...
async def test_dead_still_skips_two_sweep_without_session_sharing():
    ...
    anyone else's child and the immediate flag keeps its original warrant."""
    mgr = _make_manager(stall_idle_secs=10)
    now = 1_000.0
...
@pytest.mark.asyncio
async def test_a_done_sibling_does_not_cost_the_fast_path():
    """`_live_shared_count` counts only live agents. A finished sibling holds no
    child that could be matched, so it must not withdraw the skip."""
    mgr = _make_manager(stall_idle_secs=10)
    now = 1_000.0
    info = _info(turns=1, _pid=4242, last_activity=now - 25, _session_sharing=True)
