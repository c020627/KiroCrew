...
def _expand_job_variables(job: CronJob) -> str:
    ...
    if not job.message:
        return job.message
    try:
        values = resolve_variables(KiroCrewConfig.load(), job.agent_id or None).values
    except Exception:
...
def build_cron_session_context(job: CronJob) -> tuple[str, str]:
    ...
    return f"cron:{job.id}:{run_id}", _expand_job_variables(job)
