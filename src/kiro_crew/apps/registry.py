...
async def _move_checkout_aside(dest: Path, log_lines: list[str]) -> Path | None:
    ...
    *dest* so the caller's state is unchanged by the attempt; if that undo
    ...
    before the cancellation is re-raised. A second cancel delivered while
    settling re-raises and skips the undo -- acceptable, as the worker still
    refreshed *aside*'s mtime in-thread, so it keeps the full retention window.
    """
    ...
    try:
        ...
        # the rename ran. asyncio.wait consumes the single delivered cancel and
        # never re-raises the worker's own exception (we do not need its result,
        # only that it settled).
...
def _report_retained_stale_checkouts(
    ...
    *,
    ...
    enclosing ``finally`` — and ONLY when the caller's exit leaves
    ``durable_success`` False. *filter_restorable* must mirror that:

    - Pass ``True`` from a failure exit that precedes the ``finally``
      restore (timeout, non-zero install script, generic exception, or an
      install/update failure after a successful clone+build — none of these
      set ``durable_success``). A restorable path there is about to be put
      back, so reporting it as retained would be the exact
      misleading-recovery report this guards against.
    - Pass ``False`` from a durable-success exit (``durable_success`` is
      already ``True``), where the ``finally`` never restores anything and a
      restorable stale is genuinely retained at ``.stale-*`` until the
      age-based sweep. Filtering it there — as the previous unconditional
      filter did — is the possible-data-loss regression this parameter
      exists to prevent: the checkout would sit unreported until swept.
    """
    ...
    try:
        ...
        if install_script:
            ...
            try:
                ...
                await _kill_process_group(proc)
                # Surface any moved-aside checkout so it is not silently stranded.
                # A failure exit: the outer `finally` will restore a restorable
                # stale, so it must not also be reported as retained here.
                _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
                outcome = {
                    "ok": False,
                    "name": name,
        ...
        if is_self_managed:
            ...
            # durable_success is already True here: the outer `finally` will
            # NOT restore, so a restorable stale must still be reported or it
            # sits unlogged at `.stale-*` until the sweep deletes it.
            _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=False)
            if official_entry:
        ...
        if result.ok:
            ...
            # Install/update failed AFTER a successful clone+build.  The
            # moved-aside checkout (if any) cannot be safely restored because
            # pkg_dir now holds the freshly cloned+built tree.  Report the
            # retained path so the user (and the sweep) know about it — same
            # line the success paths use, so no stale-* is silently stranded.
            # durable_success was never set on this branch, so the outer
            # `finally` will still attempt to restore a restorable stale —
            # filter it here so it is not double-reported as retained.
            _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
        ...
        # stale, so it must not also be reported as retained here.
        _report_retained_stale_checkouts(build_result, log_lines, filter_restorable=True)
        outcome = {"ok": False, "name": name, "error": str(exc), "log": "\n".join(log_lines)}
        return outcome
    finally:
        ...
        # Re-stamp AFTER any restoration above: every `return` in this function
        # built `outcome["log"]` via `"\n".join(log_lines)` before this `finally`
        # ran, and that produced an immutable string snapshot — the restore
        # confirmation or the restore-failed WARNING just appended above would
        # otherwise never reach the log the caller actually receives, silently
        # defeating the "must be visible in the log the user sees" guarantee
        # right above. `outcome` is the SAME dict object being returned (dicts
        # are mutable, unlike the string it briefly held), so mutating its
        # "log" key here updates what the caller gets.
