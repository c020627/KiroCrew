...
`recommend_stub` and `recommend_share` are **separate outputs**. A stub keeps the backend 1:1 with the session (the same process topology as no gateway) and is what carries server-authored UI; sharing is the step that introduces co-tenancy. Weak evidence recommends the first and withholds the second. The dashboard's bulk action consults whichever of the two the global sharing switch makes true of a click, rather than re-deriving a policy from tiers.
